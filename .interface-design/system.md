...
- Home recent reviews and latest journal records render as separate horizontally scrolling `ActivityStream` carousels. Each card keeps its own natural height instead of stretching to the tallest neighbor; Home review prose and aspect previews use a tighter clamp while preserving cover, identity, rating, continuation, and social actions. Cards reserve vertical breathing room inside the overflow track so hover/focus treatments are never clipped at the top edge.
