...
- Review writing is a dedicated experience studio inside the shared journal Base UI modal. A new review always begins unrated; quick-rating state never leaks into the editor. The user explicitly chooses stars (half steps), five levels, 0–10 points, 0–100 precision, or recommend/do-not-recommend, while storage normalizes numeric methods to 0–100.
...
- The new-review studio exposes a compact horizontal history rail before its section tabs: “New” remains the active first item, while prior reviews show title, date, correctly formatted rating, and linked-journey context. Opening a prior item navigates to its canonical detail page; it never changes the create flow into an implicit edit.
- Owners can also edit an existing review in place from its activity row: the same studio form opens prefilled inside a Base UI dialog and commits through a dedicated update path. Edited reviews carry a quiet italic "edited" marker beside their date; editing never creates a duplicate entry and drafts never leak into the edit flow. Platform is a Base UI Select restricted to the game's real IGDB release platforms. Up to five of the eight aspects may be custom-created and renamed.
...
- The mobile review sheet is exactly `100dvw`; every internal form grid uses zero-minimum tracks (`minmax(0, 1fr)`) and direct children cap at the available width. Only deliberate rails such as the Markdown toolbar may scroll horizontally, so intrinsic labels and formatting groups can never widen or clip the sheet.
...
- Drawer behavior uses Base UI Dialog for focus trapping, Escape handling, focus return, and accessible overlay state.
...
- Avatar and banner uploads always pass through a Base UI crop modal. Avatar crops at 1:1 to a maximum 640px; banner crops at 3:1 to a maximum 1800px; both export compressed WebP before server upload. The modal has no scroll region: fixed header and commit footer frame one flexing inset stage, and the cropper resolves its maximum dimensions from that stage's final content box so the full image and every resize handle remain visible at desktop and mobile heights.
...
- Follow commits use “Following…” as the pending label. Unfollowing a mutual connection created within the last seven days requires a Base UI warning modal; confirming atomically removes both directions. Older mutual follows remove only the viewer’s outgoing relationship.
...
- Home shelves move in a slow, reversible continuous auto-scroll while preserving drag/touch scrolling and quiet 36px previous/next controls. The auto-moving track temporarily drops mandatory snap so motion cannot be pinned to the first card; auto-scroll pauses independently for hover, pointer interaction, or focus, stops while the tab is hidden, and is fully disabled by reduced-motion preference. Explicit pause/resume always wins over those transient interaction reasons: resuming clears them immediately, so motion restarts even while the control retains pointer hover or keyboard focus.
...
- Saved quick actions must read as state, not lighter text: active action buttons use a semantic filled surface, wishlist/backlog/favorite markers remain visible on the cover, and checked Base UI menu rows receive a tinted inset surface. Mutations stay quiet unless they fail; inline card feedback handles errors.
...
- Library removal uses a 170ms card exit followed by a 260ms View Transition reflow, with an opacity-only reduced-motion fallback. Sorting uses a Base UI Select matching the inset toolbar, and nested mobile menus keep a 12px viewport collision boundary.
...
- Every verified mark adjacent to a user identity occupies the same 24px inline slot with the same centered 18px artwork used by the profile title. Compact search, comment, moderation, list-author, account, and activity contexts may not shrink, offset, or vertically nudge that identity mark.
...
- Every share trigger uses one component contract across profiles, games, reviews, and lists: “Share” label, 38px desktop / 40px mobile height, 12px inline padding, 8px radius, quiet raised surface, and the same hover/press feedback. Context classes may position it but never restyle its dimensions or visual state. Share actions open one motion-polished Base UI choice modal: copy the canonical current URL with an animated confirmation, or send exclusively through the native Web Share sheet. When Web Share is unavailable, the send choice is visibly disabled rather than silently changing channels. Profile reports use a separate Base UI modal, write to the existing owner-readable reports table, and never render a report action against oneself.
...
- Backloggd import collects game links, personal ratings, and the exact Played, Playing, Backlog, and Wishlist memberships; validates exact slugs against IGDB; skips uncertain matches; and shows a compact cover-led conference shelf before any write. Reviews, notes, dates, lists, and journals never cross the boundary. New games retain simultaneous collection memberships and the source rating; existing local records are never overwritten.
- The import conference reads every safe pagination link from the public `/games` route and its four canonical category filters. Its preview begins with a compact 48px source identity (public avatar, display name, handle, and checked-page count), then exact totals and the cover shelf, so players can confirm both whose collection and how much of it was read. Every cover row exposes category and rating metadata. The primary import action opens a final compact confirmation with new, categorized, and rated counts before the atomic commit. The section header uses Backloggd's official square mark as a restrained source badge; public avatars from both official Backloggd CDN hosts pass through the same-origin, source-segmented, format-sniffed avatar endpoint. The initial fallback appears only when neither official source yields a valid image, so identity never collapses into a broken image.
- A confirmed import adds only missing games with their validated Played, Playing, Backlog, and Wishlist memberships plus personal rating; truly uncategorized source rows alone remain Unclassified. Existing library rows are immutable during import. The server owns a 30-minute preview session and the database consumes it atomically, so IDs cannot be changed from the client and repeated confirmation cannot duplicate games.
...
- Active filters become removable tonal chips above the workspace. Sort uses the shared Base UI surface language; result cards retain live quick actions, personalized covers, URL pagination, loading attenuation, empty recovery, and a shape-matched route skeleton.
...
- Alternatives open in a Base UI modal and identify their source as Default, Localized, or Edition.
...
- Home recent reviews and latest journal records render as separate vertical `ActivityStream` lists, preserving the shared card contract, source order, natural height, cover, identity, rating, continuation, and social actions. Automatic movement stays reserved for game shelves; every auto-moving shelf reserves a 44px mobile control band above its covers, showing only pause/resume on touch, while discovery also keeps desktop navigation clear of artwork.
...
- On mobile list and tier-list detail headers use three deliberate rows: author identity, then format plus game count, then social actions. Format metadata never competes horizontally with the author's name.
...
- Review and Journal share one Base UI modal with persistent tabs, game identity in the header, a scrollable form body, and a sticky action footer. Switching tabs never dismisses the surface.
...
- Any control meaning “today” builds its `YYYY-MM-DD` value from local calendar fields, never from UTC `toISOString()`, so late-evening players cannot be moved into tomorrow.
