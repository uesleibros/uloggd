...
- Home recent reviews and latest journal records render as separate vertical `ActivityStream` lists, preserving the shared card contract, source order, natural height, cover, identity, rating, continuation, and social actions. Automatic movement stays reserved for game shelves; discovery carousels reserve a 44px control band above their covers so pause/resume and navigation never overlap artwork at any breakpoint.
