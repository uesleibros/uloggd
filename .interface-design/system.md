...
- The refinement rail shrink-wraps short filter content and is bounded to a 620px maximum work surface, with a fixed heading, independently scrolling body, and always-reachable commit footer. Its body may shrink when content exceeds the viewport but must never grow to consume free height; the scroll range ends directly after the release-window fields. Filter edits remain a local draft until “Apply filters”; the URL and catalog request update once per batch rather than after every checkbox or field blur. Clearing inside the rail also remains a draft until committed.
