...
- The journey editor uses a start-anchored two-column workspace on desktop (summary/date access beside the calendar) and puts the calendar first on mobile. The calendar always exposes the complete month without an internal scrollbar: seven fixed columns, low non-square day cells, compact month controls, and four equal jump targets. Journey chips may use a deliberate single-line horizontal rail so they never push the month below the initial mobile viewport.
...
- Journey detail is anchored to the beginning of the product canvas, not centered. Desktop pairs the full session timeline with a narrow route map that deep-links to every numbered session and groups linked reviews; the rail collapses below the timeline on smaller screens, and the journey skeleton preserves this exact split.
