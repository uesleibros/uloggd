...
- The journey calendar is also the game's play history: existing entries render as continuous playing-blue bands across their covered days (rounded caps at true interval edges and week boundaries, a small "Jogado" label at each interval start), while the in-progress drag selection stays blurple. A quiet "Hoje" action jumps to and selects the current day.
