...
- The import conference reads every safe pagination link from the public `/games` route. Its preview begins with a compact 48px source identity (public avatar, display name, handle, and checked-page count), then exact totals and the cover shelf, so players can confirm both whose collection and how much of it was read.
