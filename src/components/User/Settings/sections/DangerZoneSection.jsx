...
export default function DangerZoneSection({ onDelete, loading }) {
  ...
  return (
    <SettingsSection title="Zona de perigo" description="Ações irreversíveis. Tenha certeza antes de prosseguir." danger>
      {!confirm ? (
        <button
        ...
        <div className="p-3 sm:p-4 bg-zinc-900/30 border border-red-500/20 rounded-lg space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
            ...
            >
              {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : "Excluir permanentemente"}
            </button>
          </div>
        </div>
      )}
