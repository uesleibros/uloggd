...
function SpoilerText({ children }) {
  ...
  return (
    <span
      ...
      className="relative inline-block cursor-pointer select-none align-middle max-w-full"
      title="Clique para revelar"
    >
      <span className="inline-block max-w-full px-2 py-0.5 rounded whitespace-normal break-words text-transparent">
        {children}
      </span>
		
      <span
        className={`absolute inset-0 rounded transition-colors flex items-center justify-center ${
          revealed ? "bg-zinc-700/40" : "bg-zinc-700 hover:bg-zinc-600"
        }`}
      >
        {revealed ? (
          <span className="px-2 py-0.5 text-zinc-300 whitespace-normal break-words max-w-full">
            {children}
          </span>
        ) : (
