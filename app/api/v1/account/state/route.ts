...
export const GET = apiRoute({
  ...
  (select username from public.profiles where id=$1) as username,
  -- Through the definer function, because the role column is revoked from
  -- the role this connection runs as. It answers for the caller alone.
  (select public.own_account_role()::text) as role`,
        [identity.profileId],
      );
      return { data: rows[0] };
    }),
