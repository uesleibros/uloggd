...
export const GET = ownedCollection({
  ...
  table: "reviews",
  columns:
    "id, public_id, igdb_id, game_slug, title, content, rating, rating_mode, recommended, mastered, replay, contains_spoilers, visibility, platform, started_on, finished_on, journey_id, created_at, updated_at, (select jsonb_build_object('title',title) from public.journeys where id=reviews.journey_id) as journeys",
