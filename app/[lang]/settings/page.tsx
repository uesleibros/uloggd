  ...
  const [{ data: profile }, { count: infractions }, { data: blockRows }] =
    await Promise.all([
      supabase
        .from("profiles")
        .select(
          "username,username_changed_at,display_name,pronouns,bio,drawer,thought,avatar_url,banner_url,birth_date,youtube_username,instagram_username,twitter_username,custom_cover_scope,profile_comment_scope",
        )
        .eq("id", user.id)
        .single(),
      supabase
