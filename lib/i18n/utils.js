...
export function formatRelativeTime(date, language) {
  ...
  const diffSec = Math.min(0, Math.floor((dateObj.getTime() - Date.now()) / 1000))
  ...
  try {
    ...
    if (absSec < 604800) return rtf.format(Math.floor(diffSec / 86400), "day")
    if (absSec < 2592000) return rtf.format(Math.floor(diffSec / 604800), "week")
