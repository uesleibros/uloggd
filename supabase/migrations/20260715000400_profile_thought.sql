alter table public.profiles
  add column if not exists thought varchar(100);

alter table public.profiles
  add constraint profiles_thought_check check (
    thought is null or (
      char_length(trim(thought)) between 1 and 100
      and thought !~ '[\r\n]'
    )
  );

grant update (thought) on public.profiles to authenticated;

drop function if exists public.update_profile_settings(text, text, text, text, text, text);
