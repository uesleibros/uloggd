...
export async function handleGet(req, res) {
  const { listId, userId, checkGameSlug, page = 1, limit = 24 } = req.query
  ...
  try {
    ...
    let lists = (data || []).map(({ list_items, ...rest }) => {
    ...
    if (checkGameSlug && lists.length > 0) {
      const listIds = lists.map(l => l.id)

      const { data: matches } = await supabase
        .from("list_items")
        .select("list_id")
        .eq("game_slug", checkGameSlug)
        .in("list_id", listIds)

      const matchSet = new Set((matches || []).map(m => m.list_id))

      lists = lists.map(l => ({
        ...l,
        has_game: matchSet.has(l.id),
      }))
    }
