...
function buildAchievements(schema, playerMap, globalPercentages) {
	...
	const achievements = schema.map(a => {
		const internalName = String(a.name).toLowerCase()
		
		const player = playerMap.get(internalName)
		const achieved = player?.achieved === 1
		
		const globalPercent = globalPercentages.get(internalName) ?? 0
		...
		return {
			...
			globalPercent: Number(globalPercent) || 0
		}
	})
...
export async function handleGameAchievements(req, res) {
	...
	try {
		...
		if (!schema?.length) {
			const empty = { achievements: [], unlocked: 0, total: 0, percentage: 0, notConnected: !steamId }
		...
		console.error("[Steam API] Failed to fetch game achievements:", e.message)
