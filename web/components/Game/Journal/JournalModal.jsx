...
export function JournalModal({ game, existingJourney, onClose, onDeleted }) {
  ...
  async function handleBulkAdd(dates) {
    ...
    const newDates = dates.filter(d => !entryMap[d])
    ...
    try {
      const promises = newDates.map(dateStr => 
        fetch("/api/journeys/@me/addEntry", {
          method: "POST",
      ...
      notify(t("entry.bulkAdded", { count: newDates.length }))
      fetchJourneyDetails()
    } catch {
  ...
  async function handleBulkRemove(dates) {
    ...
    const entriesToRemove = dates.map(d => entryMap[d]).filter(Boolean)
    ...
    try {
      const promises = entriesToRemove.map(entry => 
        fetch("/api/journeys/@me/removeEntry", {
          method: "POST",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
      ...
      await Promise.all(promises)
      notify(t("entry.bulkRemoved", { count: entriesToRemove.length }))
      fetchJourneyDetails()
    } catch {
  ...
  return (
    <div className="w-full h-full md:h-auto md:max-w-5xl md:max-h-[90vh] bg-zinc-900 md:border md:border-zinc-700 md:rounded-xl shadow-2xl flex flex-col overflow-hidden">
      <div
        className="flex items-center justify-between px-4 pb-3 border-b border-zinc-700 flex-shrink-0 md:px-6"
              ...
              <>
                <div className="flex items-center gap-2 mb-4 flex-wrap">
                  ...
                  <div className="flex items-center gap-1 ml-auto">
                    <button
                      onClick={prevMonth}
                      className="p-2.5 bg-zinc-800 border border-zinc-700 rounded-lg text-zinc-400 hover:text-white hover:border-zinc-600 transition-colors cursor-pointer"
                    >
                ...
                {bulkSaving && (
                  <div className="flex items-center gap-2 mb-4 px-3 py-2 bg-emerald-500/10 border border-emerald-500/20 rounded-lg">
                    ...
                    <span className="text-sm text-emerald-400">{t("saving")}</span>
                  </div>
                )}
                ...
                <div className="flex items-center gap-2 mt-5 flex-wrap">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-2 text-xs text-zinc-400 hover:text-white bg-zinc-800/50 hover:bg-zinc-800 rounded-lg transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                  ...
                  <button
                    onClick={jumpToToday}
                    className="flex items-center gap-1.5 px-3 py-2 text-xs text-zinc-400 hover:text-white bg-zinc-800/50 hover:bg-zinc-800 rounded-lg transition-colors cursor-pointer"
                  >
      ...
      <div
        className="flex items-center justify-end gap-2 sm:gap-3 px-4 md:px-6 py-4 border-t border-zinc-700 flex-shrink-0"
        style={{ paddingBottom: "max(1rem, env(safe-area-inset-bottom, 1rem))" }}
      >
        <button
          type="button"
          onClick={onClose}
          className="px-5 py-2.5 text-sm font-medium text-zinc-300 hover:text-white bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 rounded-lg transition-all cursor-pointer"
        >
          {t("cancel")}
        </button>
        <button
          ...
          onClick={handleSave}
          disabled={saving}
          className="px-6 py-2.5 text-sm font-medium rounded-lg transition-all flex items-center gap-2 bg-emerald-500 hover:bg-emerald-600 text-white cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
        >
