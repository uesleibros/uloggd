...
export default function DeleteTierlistModal({ isOpen, onClose, tierlist, onDeleted }) {
  const { t } = useTranslation("tierlist.delete")
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={t("title")} maxWidth="max-w-md" fullscreenMobile showMobileGrip>
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            <p className="text-sm text-zinc-300 leading-relaxed">
              {t("confirmMessage")} <span className="text-white font-semibold">&quot;{tierlist?.title}&quot;</span>?
            </p>
            <p className="text-xs text-zinc-500 mt-1">{t("irreversible")}</p>
          </div>
        </div>
        ...
        {gamesCount > 0 && (
          <div className="flex items-center gap-3 px-4 py-3 bg-amber-500/5 border border-amber-500/20 rounded-lg">
            ...
            <p className="text-xs text-amber-400">
              {t("gamesWarning", { count: gamesCount })}
            </p>
          </div>
        )}
