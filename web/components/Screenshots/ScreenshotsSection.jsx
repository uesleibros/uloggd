...
import { Camera, Plus, Trash2, EyeOff, Eye, Pencil, X, Loader2, Gamepad2, AlertTriangle } from "lucide-react"
      ...
      <div className="p-5 space-y-4">
          ...
          {selectedGame ? (
            ...
            <div className="relative">
              <input
                ...
                className="w-full px-3.5 py-2.5 bg-zinc-800 border border-zinc-700 rounded-xl text-sm text-white placeholder-zinc-500 focus:outline-none focus:border-zinc-600 transition-colors"
              />
              {gameResults.length > 0 && (
                <div className="absolute top-full left-0 right-0 mt-1 bg-zinc-900 border border-zinc-700 rounded-xl shadow-xl z-20 overflow-hidden">
                  {gameResults.map((g) => (
                    ...
                    >
                      {g.cover?.url && (
                        <img src={`https:${g.cover.url}`} alt="" className="w-5 h-7 rounded object-cover flex-shrink-0" />
                      )}
        ...
        <label className="flex items-center gap-2.5 cursor-pointer">
          <input
            ...
            className="w-4 h-4 rounded border-zinc-600 bg-zinc-800 text-indigo-500 focus:ring-0 focus:ring-offset-0 cursor-pointer"
          />
        ...
        <div className="flex gap-2.5 pt-2">
          <button
            onClick={onClose}
            disabled={uploading}
            className="flex-1 px-4 py-2.5 text-sm font-medium text-zinc-300 bg-zinc-800 hover:bg-zinc-700 rounded-xl transition-colors cursor-pointer disabled:opacity-50"
          >
            {t("upload.cancel")}
          </button>
          <button
...
function ScreenshotLightbox({ screenshot, screenshots, isOpen, onClose, onNavigate, isOwner, onEdit, onDelete }) {
  const { t } = useTranslation("screenshots")
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} maxWidth="max-w-4xl" showCloseButton={false} raw>
      <div className="relative w-full max-h-[90vh] flex flex-col bg-zinc-900 rounded-xl overflow-hidden border border-zinc-700/50">
        <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-800 flex-shrink-0">
          <div className="flex items-center gap-2 min-w-0">
            ...
            {screenshot.caption && (
              <span className="text-xs text-zinc-500 truncate">{screenshot.caption}</span>
            )}
          </div>
          <div className="flex items-center gap-1">
            {isOwner && (
              <>
                ...
                {confirmDelete ? (
                  <button
                    onClick={() => { onDelete(screenshot.id); onClose() }}
                    className="px-2 py-1 text-xs text-red-400 hover:text-red-300 bg-red-500/10 rounded-lg cursor-pointer"
                  >
            ...
            <button onClick={onClose} className="p-1.5 text-zinc-500 hover:text-white rounded-lg hover:bg-zinc-800 transition-colors cursor-pointer">
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 min-h-0 flex items-center justify-center p-4 relative">
          {isSpoilerHidden ? (
            <div className="flex flex-col items-center gap-3">
              <img src={screenshot.image_url} alt="" className="max-w-full max-h-[70vh] object-contain rounded-lg blur-xl opacity-50" />
              <button
            ...
            <img src={screenshot.image_url} alt="" className="max-w-full max-h-[70vh] object-contain rounded-lg" />
          )}

          {currentIndex > 0 && (
            <button
              onClick={handlePrev}
              className="absolute left-2 top-1/2 -translate-y-1/2 p-2 bg-black/50 hover:bg-black/70 text-white rounded-full cursor-pointer"
            >
              ‹
            </button>
          )}
          {currentIndex < screenshots.length - 1 && (
            <button
              onClick={handleNext}
              className="absolute right-2 top-1/2 -translate-y-1/2 p-2 bg-black/50 hover:bg-black/70 text-white rounded-full cursor-pointer"
            >
              ›
            </button>
          )}
        ...
        <div className="px-4 py-2.5 border-t border-zinc-800 text-xs text-zinc-600 text-center flex-shrink-0">
  ...
  return (
    <div>
      ...
      <ScreenshotLightbox
        ...
        onEdit={(s) => { setLightbox(null); setEditingScreenshot(s) }}
