...
export default function AvatarEditor({ currentAvatar, onSave, saving = false }) {
  ...
  function handleFileSelect(e) {
    ...
    if (!file.type.startsWith("image/")) {
      notify(t("avatar.errors.invalidFile"), "error")
      return
    }

    if (file.size > MAX_FILE_SIZE) {
      notify(t("avatar.errors.fileTooLarge"), "error")
      return
    }

    const reader = new FileReader()
    reader.onload = () => setCropSrc(reader.result)
    reader.onerror = () => notify(t("avatar.errors.fileReadError"), "error")
  ...
  async function handleUrlSubmit() {
    ...
    try {
      ...
      if (!contentType?.startsWith("image/")) {
        setUrlError(t("avatar.errors.invalidUrl"))
        setUrlLoading(false)
        return
      }
      ...
      if (blob.size > MAX_FILE_SIZE) {
        setUrlError(t("avatar.errors.urlTooLarge"))
        setUrlLoading(false)
        return
      }
      ...
      reader.onerror = () => {
        setUrlError(t("avatar.errors.urlProcessError"))
        setUrlLoading(false)
      }
      reader.readAsDataURL(blob)
    } catch {
      setUrlError(t("avatar.errors.urlLoadError"))
      setUrlLoading(false)
    }
  }
  ...
  function handleSave() {
    ...
    reader.onerror = () => notify(t("avatar.errors.urlProcessError"), "error")
  ...
  function handleRemove() {
    ...
    if (currentAvatar) {
      ...
      notify(t("avatar.discarded"), "info")
    }
  }
        ...
        <div className="bg-zinc-800/50 border border-zinc-700 rounded-lg p-3 sm:p-4 space-y-3">
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3">
            ...
            >
              ...
              <span className="text-sm text-zinc-300">{t("avatar.uploadFile")}</span>
            </button>
            <button
              type="button"
              onClick={() => setMode("url")}
            ...
            >
              ...
              <span className="text-sm text-zinc-300">{t("avatar.pasteUrl")}</span>
            </button>
          </div>
