...
export default function TwitchSection() {
  ...
  if (loading) {
    return (
      <SettingsSection title={t("settings.twitch.title")}>
      ...
      <div className={`p-4 rounded-lg border transition-colors ${
                ...
                {connection.isLive && connection.stream && (
                  <div className="mb-3 p-2 bg-zinc-800/50 rounded-lg border border-zinc-700/50">
                    ...
                    <div className="flex items-center gap-2 mt-1 text-[10px] text-zinc-500">
                      ...
                      <span>•</span>
                      <span>{t("settings.twitch.viewers", { count: connection.stream.viewers.toLocaleString() })}</span>
                    </div>
                  </div>
                )}

                <p className="text-xs text-zinc-500 mb-3">
                  {t("settings.twitch.connectedDescription")}
                </p>
              </>
            ) : (
              <p className="text-xs text-zinc-500 mb-3">
                {t("settings.twitch.disconnectedDescription")}
              </p>
            )}
        ...
        {!isConnected && (
          <div className="mt-4 pt-4 border-t border-zinc-700/50">
            <h4 className="text-xs font-medium text-zinc-400 mb-2">{t("settings.twitch.benefitsTitle")}</h4>
            <ul className="space-y-1.5">
              <li className="flex items-center gap-2 text-xs text-zinc-500">
                <CheckCircle2 className="w-3.5 h-3.5 text-purple-400" />
                {t("settings.twitch.benefits.liveIndicator")}
              </li>
              <li className="flex items-center gap-2 text-xs text-zinc-500">
                <CheckCircle2 className="w-3.5 h-3.5 text-purple-400" />
                {t("settings.twitch.benefits.channelLink")}
              </li>
            </ul>
          </div>
        )}
