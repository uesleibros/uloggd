...
import { cmTheme, markdownHighlightStyle, customDecorations } from "../config/editorTheme"

const MENTION_REGEX = /(?:^|[\s\n])@([a-zA-Z0-9_]{0,32})$/

export function useCodeMirror({ value, onChange, maxLength, placeholder, editorRef, onMentionQuery }) {
  ...
  useEffect(() => {
    ...
    const extensions = [
      ...
      maxLength ? EditorState.transactionFilter.of(tr => {
        ...
        if (update.selectionSet || update.docChanged) {
          ...
          if (match) {
            ...
            onMentionQueryRef.current?.({
              ...
              position: coords ? { top: coords.top, left: coords.left, bottom: coords.bottom } : null,
            })
          }
        }
      }),
  ...
  }, [container, maxLength, placeholder])
