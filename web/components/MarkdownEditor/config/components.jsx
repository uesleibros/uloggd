import { Mention, SpoilerText, SpoilerImage, AlertBox } from "../components"

const YOUTUBE_REGEX = /(?:https?:\/\/)?(?:www\.)?(?:youtube\.com\/watch\?v=|youtu\.be\/)([\w-]{11})/
const INLINE_ELEMENTS = /^(a|abbr|b|bdo|br|cite|code|em|i|img|kbd|mark|q|s|small|span|strong|sub|sup|u|del|ins|mention|spoiler)$/
...
const markdownComponents = {
  ...
  p: ({ children, node }) => {
    ...
    const hasBlock = node?.children?.some(c => 
      c.type === "element" && c.tagName !== "br" && !INLINE_ELEMENTS.test(c.tagName)
    )
  ...
  spoilerimg: (props) => <SpoilerImage {...props} />,
  ...
  pre: ({ children }) => <pre className="bg-zinc-900 border border-zinc-700 rounded-md p-4 my-4 overflow-x-auto">{children}</pre>,
  ...
  center: ({ children }) => (
    <div className="
      flex flex-col items-center text-center w-full min-w-0 overflow-hidden
  ...
  "alert-box": ({ type, children }) => <AlertBox type={type}>{children}</AlertBox>,
  desktop: ({ children }) => <div className="hidden md:block">{children}</div>,
  mobile: ({ children }) => <div className="block md:hidden">{children}</div>,
}
