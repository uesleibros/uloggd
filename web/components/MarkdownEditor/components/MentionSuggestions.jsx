...
async function fetchAllFollowing(userId) {
	...
	while (hasMore) {
		const r = await fetch("/api/users/followers", {
			...
			body: JSON.stringify({ userId, type: "following", page, limit: 100 }),
		})
...
export function MentionSuggestions({ query, position, onSelect, userId }) {
	...
	const [placement, setPlacement] = useState({ top: 0, left: 0, width: 250, opacity: 0 })
	...
	useLayoutEffect(() => {
		...
		if (top + menuHeight > window.innerHeight) top = position.top - menuHeight - 5
		if (left + menuWidth > window.innerWidth - 12) left = window.innerWidth - menuWidth - 12
		if (left < 12) left = 12

		setPlacement({ top, left, width: menuWidth, opacity: 1 })
	...
	return createPortal(
		<div
			...
			style={{ top: placement.top, left: placement.left, width: placement.width, maxHeight: 200, opacity: placement.opacity }}
		>
			{loading ? (
				...
				filtered.slice(0, 8).map(u => (
					<button
						key={u.id}
						onMouseDown={(e) => { e.preventDefault(); e.stopPropagation(); onSelect(u.username) }}
