  ...
  return (
    <div className="rounded-xl p-10 sm:p-14 bg-zinc-800/50 border border-zinc-700 flex flex-col items-center justify-center gap-4">
      ...
      <div className="text-center">
        <p className="text-sm text-zinc-400 font-medium">{t("likes.empty.title")}</p>
        <p className="text-sm text-zinc-600 mt-1">
          {isOwnProfile
            ? t("likes.empty.own")
            : t("likes.empty.other", { username })}
        </p>
      </div>
    </div>
  )
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 gap-3">
          ...
          <p className="text-sm text-zinc-600">
            {activeTab === "games"
              ? (isOwnProfile ? t("likes.emptyGames.own") : t("likes.emptyGames.other", { username }))
              : (isOwnProfile ? t("likes.emptyReviews.own") : t("likes.emptyReviews.other", { username }))
            }
          </p>
        </div>
      )}
