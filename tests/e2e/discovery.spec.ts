...
test.describe("the journey editor is closed", () => {
  test.skip(
    !canSignIn,
    "needs NEXT_PUBLIC_SUPABASE_URL, SUPABASE_SECRET_KEY and the publishable key",
  );

  const accounts2: TestAccount[] = [];
  test.afterAll(async () => {
    await Promise.all(accounts2.map((account) => destroyAccount(account)));
    accounts2.length = 0;
  });

  test("a game offers a review and a screenshot, and no longer a journey", async ({
    page,
    context,
  }) => {
    const account = await createAccount("logs");
    ...
    // The two that stayed, asserted first and on purpose. Checking only that
    // the journey button is absent would pass just as well if the whole panel
    // failed to render, which is a green test for a broken page.
    await expect(
      page.getByRole("button", { name: "Nova avaliação" }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Nova captura" }),
    ).toBeVisible();
    // The one that went. Its editor is still in the bundle, unreferenced, so
    // this is what stops it being wired back up by accident.
    await expect(
      page.getByRole("button", { name: "Registrar jornada" }),
    ).toHaveCount(0);
  });
});
